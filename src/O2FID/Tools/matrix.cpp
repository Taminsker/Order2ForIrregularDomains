    ...
    for (int k = 0; k < Nz; ++k)
        for (int j = 0; j < Ny; ++j)
            for (int i = 0; i < Nx; ++i)
            {
                ...
                if (i == 0) {
                  ListOfTriplets.push_back (Triplet (idx, idx + 1, -5./(hx * hx)));
                  ListOfTriplets.push_back (Triplet (idx, idx + 2, 4./(hx * hx)));
                  ListOfTriplets.push_back (Triplet (idx, idx + 3, -1./(hx * hx)));
                ...
                // Si le point n'est pas sur un bord en direction x : Laplacien classique
                  ...
                  ListOfTriplets.push_back (Triplet (idx,idx_L, b));
                  ListOfTriplets.push_back (Triplet (idx,idx_R, b));
                  double aprim = -2. / (hx * hx);
                }
                ...
                if (dim == DIM_2D || dim == DIM_3D)
                {
                    ...
                    if (j == 0) {
                      ListOfTriplets.push_back (Triplet (idx, idx + 1 * Nx, -5./(hy * hy)));
                      ListOfTriplets.push_back (Triplet (idx, idx + 2 * Nx, 4./(hy * hy)));
                      ListOfTriplets.push_back (Triplet (idx, idx + 3 * Nx, -1./(hy * hy)));
                      aprim += 2. / (hy * hy);
                    }
                    ...
                    // Si le point n'est pas sur un bord en direction y : Laplacien classique
                      ...
                      ListOfTriplets.push_back (Triplet (idx,idx_F, c));
                    ...
                    if (dim == DIM_3D)
                    {
                        ...
                        if (k == 0) {
                          ListOfTriplets.push_back (Triplet (idx, idx + 1 * Nx * Ny, -5./(hy * hy)));
                          ListOfTriplets.push_back (Triplet (idx, idx + 2 * Nx * Ny, 4./(hy * hy)));
                          ListOfTriplets.push_back (Triplet (idx, idx + 3 * Nx * Ny, -1./(hy * hy)));
                          aprim += 2. / (hz * hz);
                        }
                        ...
                        // Si le point n'est pas sur un bord en direction z : Laplacien classique
                          ...
                          ListOfTriplets.push_back (Triplet (idx,idx_D, d));
                          ListOfTriplets.push_back (Triplet (idx,idx_U, d));
                          aprim += -2. / (hz * hz);
                        }
                    } // fin if dim 3
                } // fin if dim 2 ou 3

                ListOfTriplets.push_back (Triplet (idx,idx, aprim));
            } // fin for i,j,k
