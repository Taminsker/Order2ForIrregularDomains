...
int main(int argc, char* argv[])
{
    ...
    for (size_t idx = 0; idx < listNx.size (); ++idx)
    {
        ...
        /* Notation :
         * CK = crank-nicolson
         * BE1 = Backward-Euler dt = dx
         * BE2 = Backward-Euler dt = dx*dx
         */
        ...
        for (int idt = 0; idt < Nt; idt++) // boucle sur les points temporelles
        {
            ...
            Writer writer (mesh);
            writer.SetFilename (std::string ("example_7_Crank_Nicolson_Nx_") + std::to_string (Nx) + std::string ("_dt_") + std::to_string (idt*dt));
        ...
        for (int idt = 0; idt < Nt; idt++) // boucle sur les points temporelles
        {
            ...
            // Écriture dans des fichiers
            Writer writer (mesh);
            writer.SetFilename (std::string ("example_7_Backward_Euler1_Nx_") + std::to_string (Nx) + std::string ("_dt_") + std::to_string (idt*dt));
            writer.SetCurrentIteration (idt); // Itérations lorsqu'il y a du temps
        ...
        for (int idt = 0; idt < Nt; idt++) // boucle sur les points temporelles
        {
            ...
            // Écriture dans des fichiers
            Writer writer (mesh);
            writer.SetFilename (std::string ("example_7_Backward_Euler2_Nx_") + std::to_string (Nx) + std::string ("_dt_") + std::to_string (idt*dt_BE2));
            writer.SetCurrentIteration (idt); // Itérations lorsqu'il y a du temps
