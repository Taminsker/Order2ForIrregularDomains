...
{
    return std::exp(- M_PI * M_PI * t) * std::cos(M_PI * a.x);
}
...
void Make(Point* minima, Point* extrema, double time_final, STYLE style, std::vector<int> listNx, std::vector<int> listNy, std::vector<int> listNz)
{
    ...
    for (size_t i = 0; i < n; ++i)
    {
        ...
        Writer writer (mesh);
        writer.SetFilename (std::string("Example_7_") + name + std::string ("_Nx_") + std::to_string (Nx) + std::string ("_Ny_") + std::to_string (Ny) + std::string ("_Nz_") + std::to_string (Nz));
        ...
        {
            std::cout.clear();
            std::cout << "\r" << INDENT << "Iteration : time : " << t << "/" << time_final << " dt : " << dt << std::flush;
            std::cout.setstate(std::ios_base::failbit);
